...
void insert2(int hash_table[], int key, int value, int array_size) {
    int index = hash_function2(key, array_size);
...
void myDiv() {
    ...
    for (int i = 0; i < num_pairs; ++i) {
        ...
        insert2(hash_table, key, value, array_size);
    }
    ...
    int found_value = get2(hash_table, key_to_find, array_size);
    printf("키 %d에 해당하는 값: %d\n", key_to_find, found_value);

    printf("삭제할 키를 입력하세요: ");
    int key_to_remove;
    scanf_s("%d", &key_to_remove);
    remove_key2(hash_table, key_to_remove, array_size);

    printf("%d가 삭제되었는지 확인합니다.: %s\n", key_to_remove, is_empty2(hash_table, key_to_remove, array_size) ? "Yes" : "No");

    print_table2(hash_table, array_size);
